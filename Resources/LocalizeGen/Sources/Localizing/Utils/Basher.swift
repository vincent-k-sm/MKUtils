...
public struct Basher {
    ...
    public static func run(command: String, from url: URL?) throws -> String {
        ...
        if let url = url {
//            CodegenLogger.log(url.absoluteString, logLevel: .warning)
//            task.currentDirectoryURL = URL(string: url.absoluteString)
            task.currentDirectoryURL = URL(fileURLWithPath: url.path, isDirectory: true)
//            task.currentDirectoryURL = URL(fileURLWithPath: url.absoluteString)
        }
        try task.run()
